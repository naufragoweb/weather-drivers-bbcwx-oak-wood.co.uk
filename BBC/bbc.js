...
var Driver = class Driver extends wxBase.Driver {
    ...
    _loadMetaFromID(jsonlc) {
            ...
            try {
                ...
                for (let i = 0; i < this.maxDays; i++) {
                    ...
                    if (!forecastDay) {
                         ...
                         // If forecast data is missing for this day, the other properties will remain empty,
                         // but the day object with the name is still created and pushed.
        ...
        _getDayName(index) {
        // Use as abreviações que correspondem às chaves em desklet.js's this.daynames
        const dayNamesAbbr = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
        const today = new Date();
        const currentDay = today.getDay(); // 0=Sunday, 1=Monday, ..., 6=Saturday
        const dayIndex = (currentDay + index) % 7;
        return dayNamesAbbr[dayIndex]; // Retorna a abreviação do dia
    }
